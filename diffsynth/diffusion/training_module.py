...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def switch_pipe_to_training_mode(
        ...
        if lora_base_model is not None and not task.endswith(":data_process"):
            ...
            # ========== 修改这里：支持多个模型 ==========
            lora_base_models = lora_base_model.split(",")  # 分割成列表

            for model_name in lora_base_models:
                model_name = model_name.strip()  # 去除空格
                if (not hasattr(pipe, model_name)) or getattr(pipe, model_name) is None:
                    print(f"No {model_name} models in the pipeline. We cannot patch LoRA on the model. If this occurs during the data processing stage, it is normal.")
                ...
                if lora_checkpoint is not None:
                    ...
                    model_state_dict = {k: v for k, v in state_dict.items() if model_name in k or k.startswith(model_name)}

                    if model_state_dict:
                        load_result = model.load_state_dict(model_state_dict, strict=False)
                        print(f"LoRA checkpoint loaded for {model_name}: {lora_checkpoint}, total {len(model_state_dict)} keys")
    ...
    def split_pipeline_units(self, task, pipe, trainable_models=None, lora_base_model=None):
        ...
        if lora_base_model is not None:
            # ========== 修改这里：支持多个模型 ==========
            models_require_backward += lora_base_model.split(",")  # 分割成列表
            # ========== 修改结束 ==========
