...
# ★ 修改 __init__ 的 input_params 和 output_params:

class WanVideoUnit_Geo3DCoords(PipelineUnit):
    PIXEL_TO_LATENT_MAP = [0, 3, 7, 11, 15]

    def __init__(self):
        super().__init__(
            input_params=("coords_3d", "num_frames", "quality_mask"),  # ★ 加 quality_mask
            output_params=("geo3d_noisy_coords", "geo3d_ref_coords",
                           "geo3d_clean_noisy_mask"),  # ★ 加 clean_mask
        )

    def process(self, pipe, coords_3d, num_frames, quality_mask=None):  # ★ 加 quality_mask 参数
        if coords_3d is None:
            return {}

        if isinstance(coords_3d, str):
            data = np.load(coords_3d)
        else:
            data = coords_3d

        noisy_coords_all = data['world_coords_log_radial']
        num_latent_frames = (num_frames - 1) // 4 + 1
        indices = self.PIXEL_TO_LATENT_MAP[:num_latent_frames]
        noisy_coords = noisy_coords_all[indices]
        noisy_coords_flat = noisy_coords.reshape(-1, 3)

        ref_coords = data.get('ref_coords_log_radial', None)
        if ref_coords is not None:
            ref_coords_flat = ref_coords.reshape(-1, 3)
        else:
            ref_coords_flat = np.zeros((0, 3), dtype=np.float32)

        device = pipe.device
        dtype = pipe.torch_dtype
        geo3d_noisy = torch.from_numpy(noisy_coords_flat).unsqueeze(0).to(device=device, dtype=dtype)
        geo3d_ref = torch.from_numpy(ref_coords_flat).unsqueeze(0).to(device=device, dtype=dtype)

        # ★ 新增: 构建清晰帧 mask
        clean_mask = None
        if quality_mask is not None and len(quality_mask) > 0:
            from  ..pipelines.geo3d_bias import Geo3DAnchorAttentionBias
            tokens_per_frame = 35 * 64
            clean_mask = Geo3DAnchorAttentionBias.build_clean_noisy_mask(
                quality_mask_frames=quality_mask,
                num_latent_frames=num_latent_frames,
                tokens_per_frame=tokens_per_frame,
            ).to(device)
            n_clean = clean_mask.sum().item()
            print(f"  [Geo3D] clean tokens: {n_clean}/{clean_mask.shape[0]}")

        print(f"  [Geo3D] noisy: {geo3d_noisy.shape}, ref: {geo3d_ref.shape}")

        return {
            "geo3d_noisy_coords": geo3d_noisy,
            "geo3d_ref_coords": geo3d_ref,
            "geo3d_clean_noisy_mask": clean_mask,  # ★ 新增
        }
...
def model_fn_wan_video(
    ...
    # ====== 3D Bias Parameters ======
    geo3d_noisy_coords=None,
    geo3d_ref_coords=None,
    geo3d_bias_module=None,
    geo3d_clean_noisy_mask: Optional[torch.Tensor] = None,
    geo3d_multiscale_noisy: Optional[Dict[str, "torch.Tensor"]] = None,
    geo3d_multiscale_ref: Optional[Dict[str, "torch.Tensor"]] = None,
    **kwargs,
):
    import torch
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            dit=dit, motion_controller=motion_controller, vace=vace,
            latents=latents, timestep=timestep, context=context,
            clip_feature=clip_feature, y=y, reference_latents=reference_latents,
            vace_context=vace_context, vace_scale=vace_scale,
            tea_cache=tea_cache, use_unified_sequence_parallel=use_unified_sequence_parallel,
            motion_bucket_id=motion_bucket_id,
        )
    ...
    if dit.seperated_timestep and fuse_vae_embedding_in_latents:
        timestep_emb = torch.concat([
            torch.zeros((1, latents.shape[3] * latents.shape[4] // 4), dtype=latents.dtype, device=latents.device),
            torch.ones((latents.shape[2] - 1, latents.shape[3] * latents.shape[4] // 4), dtype=latents.dtype, device=latents.device) * timestep
        ]).flatten()
        t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, timestep_emb).unsqueeze(0))
    ...
    # ====================================================================
    # ★★★ 3D Attention Bias Calculation (V3.1 CPU Offload Fix) ★★★
    # ====================================================================
    # ====================================================================
    # ★★★ 3D Attention Bias Calculation (V3.1 Memory Optimized) ★★★
    # ====================================================================
    geo3d_attn_bias = None
    _geo3d_base_bias = None
    _use_v3_per_layer = False

    if (geo3d_bias_module is not None and
        geo3d_noisy_coords is not None and
        geo3d_ref_coords is not None):

        from ..pipelines.geo3d_bias import pad_bias_for_ref_queries
        
        # 1. Align Coords (Handle slicing if .npz has more frames)
        current_seq_len = x.shape[1]
        n_noisy_tokens = geo3d_noisy_coords.shape[1]
        n_ref_tokens_actual = current_seq_len - n_noisy_tokens
        
        if n_ref_tokens_actual < geo3d_ref_coords.shape[1]:
            if n_ref_tokens_actual > 0:
                geo3d_ref_coords = geo3d_ref_coords[:, :n_ref_tokens_actual, :]
            else:
                geo3d_ref_coords = geo3d_ref_coords[:, :0]
        
        if geo3d_ref_coords.shape[1] > 0:
            if hasattr(geo3d_bias_module, 'compute_base_bias'):
                _use_v3_per_layer = True
                
                # ★★★ FORCE SINGLE HEAD MODE ★★★
                # Generates [B, 1, N_noisy, N_ref] (~9MB) instead of [B, 40, ...] (~360MB)
                # Padded to square it will be ~360MB instead of ~14GB.
                _geo3d_base_bias = geo3d_bias_module.compute_base_bias(
                    geo3d_noisy_coords, geo3d_ref_coords,
                    clean_noisy_mask=geo3d_clean_noisy_mask,
                    compute_self_bias=False,
                    single_head_mode=True, # Enable memory saving
                )
                
                # Pad Rows (Ref queries): [B, 1, N_all, N_ref]
                _geo3d_base_bias = pad_bias_for_ref_queries(
                    _geo3d_base_bias, n_ref_tokens=n_ref_tokens_actual)
                
                # Pad Columns to Square: [B, 1, N_all, N_all]
                # Broadcasting will handle 1 -> 40 heads in SDPA
                if _geo3d_base_bias.shape[-1] < current_seq_len:
                    padding_col = current_seq_len - _geo3d_base_bias.shape[-1]
                    _geo3d_base_bias = F.pad(_geo3d_base_bias, (0, padding_col), value=0.0)
                
                # Offload to CPU just in case, though 360MB might fit
                _geo3d_base_bias = _geo3d_base_bias.detach().cpu()
                
            else:
                # Legacy path (unlikely to be hit but kept for safety)
                geo3d_attn_bias = geo3d_bias_module.forward_cross_only(
                    geo3d_noisy_coords, geo3d_ref_coords)
                geo3d_attn_bias = pad_bias_for_ref_queries(
                    geo3d_attn_bias, n_ref_tokens=n_ref_tokens_actual)
                if geo3d_attn_bias.shape[-1] < current_seq_len:
                    padding_col = current_seq_len - geo3d_attn_bias.shape[-1]
                    geo3d_attn_bias = F.pad(geo3d_attn_bias, (0, padding_col), value=0.0)
    # ====================================================================
    ...
    if tea_cache_update:
        ...
        def create_custom_forward_with_bias(module, attn_bias):
            def custom_forward(x, context, t_mod, freqs):
                return module(x, context, t_mod, freqs, attn_bias=attn_bias)
            return custom_forward
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            # Standard path
                ...
                # Standard Path
                if vap is None or block_id not in vap.mot_layers_mapping:
                    if _use_v3_per_layer:
                        # Apply gate (moves small 360MB tensor to GPU)
                        current_attn_bias = geo3d_bias_module.apply_layer_gate(
                            _geo3d_base_bias, layer_idx=block_id)
                        current_attn_bias = current_attn_bias.detach()
                    else:
                        current_attn_bias = geo3d_attn_bias

                    if use_gradient_checkpointing_offload:
                        with torch.autograd.graph.save_on_cpu():
                            x = torch.utils.checkpoint.checkpoint(
                                create_custom_forward_with_bias(block, current_attn_bias),
                                x, context, t_mod, freqs, use_reentrant=False)
                    elif use_gradient_checkpointing:
                        x = torch.utils.checkpoint.checkpoint(
                            create_custom_forward_with_bias(block, current_attn_bias),
                            x, context, t_mod, freqs, use_reentrant=False)
                    else:
                        x = block(x, context, t_mod, freqs, attn_bias=current_attn_bias)
                                        # ★ 用完立即释放 GPU 上的 bias
                    if _use_v3_per_layer and current_attn_bias is not None:
                        del current_attn_bias
