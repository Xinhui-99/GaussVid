...
def random_sample(c2ws_anchor, H, W, fs, c, phi, theta, r, frame, device, viz_traj=False, save_dir = None):
    # Initialize a camera.
    """
    The camera coordinate sysmte in COLMAP is right-down-forward
    Pytorch3D is left-up-forward
    """
    phis = txt_interpolation(phi, frame, mode='random')
    thetas = txt_interpolation(theta, frame, mode='random')
    rs_o = txt_interpolation(r, frame, mode='random')
    rs = rs_o * c2ws_anchor[0,2,3].cpu().numpy()
    ...
    image_size = ((H, W),)  # (h, w)
    cameras = PerspectiveCameras(focal_length=fs, principal_point=c, in_ndc=False, image_size=image_size, R=R_new, T=T_new, device=device)
    return cameras, num_views, phis, thetas, rs_o


def spherical_relative_pose(anchor_c2w: torch.Tensor, delta_theta: float, delta_phi: float, device='cuda'):
    """
    Rotate the anchor camera on the sphere centered at its look-at target.
    The rotation is relative to the anchor, based on spherical motion.
    
    Args:
        anchor_c2w: [1, 4, 4] anchor camera pose, OpenGL (Z forward)
        delta_theta: azimuthal change (in degrees)
        delta_phi: elevation change (in degrees)
    
    Returns:
        new_c2w: [1, 4, 4] new pose after spherical rotation
    """
    anchor_c2w = anchor_c2w.to(device)
    cam_pos = anchor_c2w[0, :3, 3]                 # camera position
    forward = -anchor_c2w[0, :3, 2]                # OpenGL: -Z forward
    target = cam_pos + forward                    # look-at point
    rel = cam_pos - target                         # vector from target to camera
    ...
    R = R_x @ R_y
    new_rel = torch.tensor(R @ rel_np, dtype=torch.float32, device=device)

    new_pos = target + new_rel
    new_forward = F.normalize(target - new_pos, dim=0)
    up = torch.tensor([0, 1, 0], dtype=torch.float32, device=device)
    right = F.normalize(torch.cross(new_forward, up), dim=0)
    up = F.normalize(torch.cross(right, new_forward), dim=0)

    new_c2w = torch.eye(4, device=device)
    new_c2w[:3, 0] = right
    new_c2w[:3, 1] = up
    new_c2w[:3, 2] = -new_forward  # OpenGL
    new_c2w[:3, 3] = new_pos
    return new_c2w.unsqueeze(0)
...
def random_sample_poses(c2ws_anchor, phi, theta, r, frame, device):
    phis = txt_interpolation(phi, frame, mode='random')
    thetas = txt_interpolation(theta, frame, mode='random')
    rs_o = txt_interpolation(r, frame, mode='random')
    rs = rs_o * c2ws_anchor[0,2,3].cpu().numpy()
...
def world_point_to_kth(poses, points, k, device):
    ...
    inv_kth_pose_batch = inv_kth_pose.unsqueeze(0).repeat(N, 1, 1)
    homogeneous_points = torch.cat([points, torch.ones(N, W*H, 1).to(device)], dim=-1)  
    new_points = inv_kth_pose_batch.unsqueeze(1)@ homogeneous_points.unsqueeze(-1)
...
def convert_opencv_to_nerf_c2w(c2w: torch.Tensor) -> torch.Tensor:
    """
    将 OpenCV 格式的 c2w 变换为 NeRF/OpenGL 格式的 c2w。
    假设输入是 [B, 4, 4] 或 [4, 4] 的 torch.Tensor。
    """
    assert c2w.shape[-2:] == (4, 4), "Invalid c2w shape"
    single = False
    if c2w.ndim == 2:
        c2w = c2w.unsqueeze(0)
        single = True
    ...
    cv_to_gl = torch.diag(torch.tensor([1, -1, -1, 1], dtype=c2w.dtype, device=c2w.device)).unsqueeze(0)  # [1, 4, 4]


    c2w_nerf = torch.bmm(cv_to_gl, c2w)

    return c2w_nerf[0] if single else c2w_nerf

def convert_opencv_to_nerf_pointmap(pointmap: torch.Tensor) -> torch.Tensor:
    """
    将 [H, W, 3] 或 [B, H, W, 3] 格式的 OpenCV 世界坐标点转换到 NeRF 坐标系下。

    OpenCV -> NeRF: y, z 取负
    """
    if pointmap.ndim == 3:
        x, y, z = pointmap.unbind(-1)
        pointmap = torch.stack([x, -y, -z], dim=-1)
    elif pointmap.ndim == 4:
        x, y, z = pointmap.unbind(-1)
        pointmap = torch.stack([x, -y, -z], dim=-1)
    else:
        raise ValueError("Invalid pointmap shape")
    return pointmap
