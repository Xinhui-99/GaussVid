...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
        ...
        return {"noise": noise, "num_reference_frames": f if vace_reference_image is not None else 0}
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if vace_context is not None:
        # === 新增：维度对齐 ===
        latents_time_dim = latents.shape[2]
        vace_time_dim = vace_context.shape[2]
        if vace_time_dim != latents_time_dim:
            if vace_time_dim < latents_time_dim:
                vace_context = torch.nn.functional.pad(
                    vace_context, (0, 0, 0, 0, 0, latents_time_dim - vace_time_dim), 
                    mode='replicate'
                )
            else:
                vace_context = vace_context[:, :, :latents_time_dim]
