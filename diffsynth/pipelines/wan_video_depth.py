...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        use_first_last_frame_guidance: Optional[bool] = True,
        ...
        # VACE (TODO: remove it)
        _ref_for_trim = vace_reference_image or inputs_shared.get("ref_prefix_images")
        if _ref_for_trim is not None or (animate_pose_video is not None and animate_face_video is not None):
            if _ref_for_trim is not None and isinstance(_ref_for_trim, list):
                f = len(_ref_for_trim)
            elif _ref_for_trim is not None:
                f = 1
            else:
                f = 1
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", "seed", "rand_device", "ref_prefix_images"),
            output_params=("noise",)
        )

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, ref_prefix_images):
        length = (num_frames - 1) // 4 + 1
        if ref_prefix_images is not None:
            f = len(ref_prefix_images) if isinstance(ref_prefix_images, list) else 1
            length += f
...
class WanVideoUnit_DenoiseCond(PipelineUnit):
    """把noisy video通过VAE encode后写入y通道，用于video denoising任务。
    支持 quality_mask：根据帧质量（清晰/噪声）设置不同的 mask 置信度。
    
    贝叶斯动机：
        mask 值编码条件似然 p(x_noisy | x_clean) 的精度 (precision)。
        清晰帧：p(x_noisy|x_clean) ≈ δ(x_noisy - x_clean) → mask=1.0（完全可信）
        噪声帧：p(x_noisy|x_clean) 是宽分布 → mask=QUALITY_VALUE（部分可信）
        
        这让模型在去噪时更多依赖清晰帧的条件信息，
        对噪声帧的条件信息保持适度怀疑。
    
    放在 ImageEmbedderVAE 之后，会覆盖其产生的 y。
    """
    
    # 噪声帧的条件置信度（可配置）
    # 0.0 = 完全不信任噪声帧（退化为纯生成）
    # 0.3 = 部分信任（推荐值，保留色彩/结构信息但降低权重）
    # 1.0 = 完全信任（等价于原始全1 mask，不区分清晰/噪声）
    QUALITY_VALUE = 0.3

    def __init__(self):
        super().__init__(
            input_params=("noisy_video", "quality_mask", "num_frames", "height", "width",
                          "tiled", "tile_size", "tile_stride"),
            output_params=("y",),
            onload_model_names=("vae",)
        )

    def _parse_frame_quality(self, quality_mask, num_frames):
        """
        解析 quality_mask（PIL Image 列表）→ 逐帧置信度列表。
        
        mask_video 中的帧：
            像素值 ≈ 0   → 清晰帧（GT质量）→ 置信度 1.0
            像素值 ≈ 255 → 噪声帧（3DGS渲染）→ 置信度 QUALITY_VALUE
        
        Returns:
            List[float]: 每帧的置信度值，长度 = num_frames
        """
        import numpy as np
        confidences = []
        for i in range(num_frames):
            if i < len(quality_mask):
                mask_arr = np.array(quality_mask[i])
                mean_val = mask_arr.mean()
                # 阈值128：干净区分 0 vs 255
                if mean_val > 128:
                    confidences.append(self.QUALITY_VALUE)
                else:
    ...
    def process(self, pipe, noisy_video, quality_mask, num_frames, height, width,
                tiled, tile_size, tile_stride):
        if noisy_video is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)

        # ========== VAE encode noisy video → [1, 16, T', H', W'] ==========
        noisy_tensor = pipe.preprocess_video(noisy_video)
        noisy_latents = pipe.vae.encode(
            noisy_tensor, device=pipe.device,
            tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        ...
        if quality_mask is not None and len(quality_mask) > 0:
            # 逐帧解析置信度
            confidences = self._parse_frame_quality(quality_mask, num_frames)
            msk = torch.ones(1, num_frames, lat_h, lat_w, device=pipe.device)
            for i, conf in enumerate(confidences):
                msk[:, i] = conf
            
            n_clean = sum(1 for c in confidences if c >= 1.0)
            n_noisy = num_frames - n_clean
            print(f"  [QualityMask] {n_clean} clean (1.0) + {n_noisy} noisy ({self.QUALITY_VALUE}) frames")
        else:
            # fallback：全1 mask（兼容无 quality_mask 的场景）
            msk = torch.ones(1, num_frames, lat_h, lat_w, device=pipe.device)

        # ========== VAE temporal compression ==========
        # 第0帧 repeat 4次（与 WAN2.1 的 temporal causal conv 一致）
        msk = torch.concat([
        ...
        # ========== 拼接: [4ch_mask, 16ch_noisy_latent] → [20ch] ==========
        y = torch.concat([msk, noisy_latents[0]])  # [20, T', H', W']
        y = y.unsqueeze(0).to(dtype=pipe.torch_dtype, device=pipe.device)
        return {"y": y}
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "ref_prefix_images"),
            output_params=("latents", "input_latents"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, ref_prefix_images):
...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            # ========== 修改 mask 生成逻辑 ==========
            use_first_last_frame_guidance = True
            if vace_video_mask is None:
                if use_first_last_frame_guidance:
                    # 首尾帧引导模式：首尾帧为0，中间帧为1
                    vace_video_mask = torch.ones_like(vace_video)
                    vace_video_mask[:, :, 0] = 0    # 首帧已知
                    vace_video_mask[:, :, -1] = 0   # 尾帧已知
                else:
                    vace_video_mask = torch.ones_like(vace_video)
            else:
                vace_video_mask = pipe.preprocess_video(vace_video_mask, min_value=0, max_value=1)
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if y is not None and dit.require_vae_embedding:
        # When vace_reference_image is used, latents have extra frames that y doesn't
        if x.shape[2] != y.shape[2]:
            pad_size = x.shape[2] - y.shape[2]
            y = torch.cat([
                torch.zeros(*y.shape[:2], pad_size, *y.shape[3:], device=y.device, dtype=y.dtype),
                y
            ], dim=2)
        x = torch.cat([x, y], dim=1)
    ...
    if vace_context is not None and vace is not None:
        ...
        vace_context = None  # 强制禁用后续 VACE hint 注入
