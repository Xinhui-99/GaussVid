...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if "last_frame_latents" in inputs_shared:
                inputs_shared["latents"][:, :, -1:] = inputs_shared["last_frame_latents"]
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    ...
    def process(self, pipe, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        ...
        if end_image is not None:
            ...
            # ✅ 编码最后一帧的 latent
            end_image_processed = end_image.transpose(0, 1)  # 转换维度
            end_latent = pipe.vae.encode(
                [end_image_processed.to(dtype=pipe.torch_dtype, device=pipe.device)], 
                device=pipe.device, 
                tiled=tiled, 
                tile_size=tile_size, 
        ...
        # ✅ 返回 last_frame_latents
        result = {"y": y}
        if end_latent is not None:
            result["last_frame_latents"] = end_latent
        return result
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def __init__(self):
        super().__init__(
            input_params=("input_image", "end_image", "latents", "height", "width", "tiled", "tile_size", "tile_stride"),  
            output_params=("latents", "fuse_vae_embedding_in_latents", "first_frame_latents", "last_frame_latents"),  
            onload_model_names=("vae",)
        )
    
    def process(self, pipe, input_image, end_image, latents, height, width, tiled, tile_size, tile_stride):
        ...
        if end_image is not None:
            end_image_tensor = pipe.preprocess_image(end_image.resize((width, height))).transpose(0, 1)
            end_z = pipe.vae.encode([end_image_tensor], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            latents[:, :, -1:] = end_z
            result["last_frame_latents"] = end_z
        else:
            # 如果没有 end_image，也保存当前最后一帧的 latent
            result["last_frame_latents"] = latents[:, :, -1:].clone()
...
class WanVideoUnit_VACE(PipelineUnit):
    def __init__(self):
        super().__init__(
            ...
            output_params=("vace_context", "vace_scale", "first_frame_latents", "last_frame_latents"),  
            onload_model_names=("vae",)
        )
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            # ========== 生成mask：首尾帧为0（已知），中间帧为1（未知） ==========
            vace_video_mask = torch.ones_like(vace_video)
            vace_video_mask[:, :, 0] = 0    # 首帧已知（清晰）
            vace_video_mask[:, :, -1] = 0   # 尾帧已知（清晰）
            # 中间帧保持为1（模糊，需要修复）

            # ========== 编码vace_video ==========
            inactive = vace_video * (1 - vace_video_mask)  # 已知区域（首尾清晰帧）
            reactive = vace_video * vace_video_mask        # 未知区域（中间模糊帧）
            ...
            # ========== 提取首尾帧latents用于推理时锁定 ==========
            first_frame_latents = inactive[:, :, 0:1]  # [1, 16, 1, H, W]
            last_frame_latents = inactive[:, :, -1:]   # [1, 16, 1, H, W]
            
            # ========== 构造vace_context ==========
            vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1)  # [1, 33, 2, H, W]
