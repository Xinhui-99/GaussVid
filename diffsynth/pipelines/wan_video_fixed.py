...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # ★★★ 新增: Video Denoising ★★★
        noisy_video: Optional[list[Image.Image]] = None,       # → DenoiseCond y通道
        ref_prefix_images: Optional[list[Image.Image]] = None, # → 序列前缀参考帧
        # ControlNet
...
class WanVideoUnit_DenoiseCond(PipelineUnit):
    ...
    def __init__(self):
        super().__init__(
            input_params=("noisy_video", "num_frames", "height", "width",
                          "tiled", "tile_size", "tile_stride"),
            output_params=("y",),
            onload_model_names=("vae",)
        )

    def process(self, pipe, noisy_video, num_frames, height, width,
                tiled, tile_size, tile_stride):
        ...
        # mask 全1：表示所有帧都提供了条件信息
        lat_h, lat_w = height // 8, width // 8
        msk = torch.ones(1, num_frames, lat_h, lat_w, device=pipe.device)* 0.5
        # 首帧: 1.0（干净参考）
        msk[:, 0] = 1.0
        # 尾帧: 1.0（干净参考）
        msk[:, -1] = 1.0
        ...
        msk = msk.transpose(1, 2)[0]  # [1, T', H', W']

        y = torch.concat([msk, noisy_latents[0]])  # [17, T', H', W']
