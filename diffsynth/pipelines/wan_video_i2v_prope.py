...
from ..models.wan_video_dit_prope import WanModel, sinusoidal_embedding_1d
...
"""
wan_video_i2v.py 的修改清单:

1. 新增 import
2. 新增 WanVideoUnit_PropeComputer PipelineUnit
3. 在 WanVideoPipeline.__init__ 的 units 列表中插入该 unit
4. 修改 model_fn_wan_video 以使用 prope_data
...
# 修改 1: 新增 import (文件顶部)
# ============================================================
# 在 wan_video_i2v.py 的 import 区域添加:
from ..models.prope_utils import compute_multi_camera_prope_data, PropeData
from ..models.wan_video_dit_prope import set_prope_data_on_blocks, clear_prope_data_on_blocks


# ============================================================
# 修改 2: 新增 PipelineUnit
# ============================================================

class WanVideoUnit_PropeComputer(PipelineUnit):
    """
    计算 PRoPE 数据（Sub-Block Multi-Camera 方案）。
    
    位置：放在 WanVideoUnit_VACE 之后、WanVideoUnit_CfgMerger 之前。
    
    工作流程:
      1. 读取 vace_camera 中的逐帧 c2w 和 K
      2. 将像素帧映射到 latent 帧（保留全部 4 个 sub-camera）
      3. 计算投影矩阵和 2D RoPE 系数
      4. 输出 prope_data 对象
    
    当 vace_camera 为 None 时，不输出任何内容，
    model_fn 中 prope_data 为 None，完全回退到原始 3D RoPE。
    """
    def __init__(self):
        super().__init__(
            input_params=(
                "vace_camera", "height", "width", "num_frames",
                "vace_reference_image",
            ),
            output_params=("prope_data",),
        )

    def process(
        self, pipe: 'WanVideoPipeline',
        vace_camera, height, width, num_frames, vace_reference_image
    ):
        if vace_camera is None:
            return {}
        ...
        num_latent_frames = (num_frames - 1) // 4 + 1
        num_heads = getattr(pipe.dit, 'num_heads', None) or pipe.dit.blocks[0].num_heads
        head_dim = pipe.dit.dim // num_heads
        
        # 检查 head_dim 是否兼容 PRoPE
        if head_dim % 4 != 0:
            print(f"[PRoPE] Warning: head_dim={head_dim} not divisible by 4, "
                  f"skipping PRoPE")
            return {}
        
        # 计算 patchify 后的空间分辨率
        vae_factor = pipe.vae.upsampling_factor if pipe.vae is not None else 8
        patch_size = pipe.dit.patch_size  # (t, h, w), 通常 (1, 2, 2)
        patches_h = height // vae_factor // patch_size[1]
        patches_w = width // vae_factor // patch_size[2]
        
        # 是否有 reference image (会在序列前面多一帧)
        has_ref = vace_reference_image is not None
        
        try:
            prope_data = compute_multi_camera_prope_data(
                pixel_c2ws=vace_camera["c2ws"],
                pixel_Ks=vace_camera["Ks"],
                num_latent_frames=num_latent_frames,
                patches_h=patches_h,
                patches_w=patches_w,
                image_height=height,
                ...
                num_heads=num_heads,
                device=pipe.device,
                has_reference_latents=has_ref,
            )
            print(f"[PRoPE] Computed: cameras={num_latent_frames + (1 if has_ref else 0)}, "
                  f"patches=({patches_h},{patches_w}), head_dim={head_dim}")
            return {"prope_data": prope_data}
        except Exception as e:
            print(f"[PRoPE] Failed to compute: {e}, falling back to 3D RoPE")
            import traceback
            traceback.print_exc()
            return {}
...
def model_fn_wan_video(
    ...
    prope_data=None,                       # ★ 新增参数
    **kwargs,
):
    # === sliding_window 分支: 需要把 prope_data 也传进去 ===
    ...
    # Camera control
    x = dit.patchify(x, control_camera_latents_input=None)
    ...
    if vace_context is not None:
     ...
     # ═══════════════════════════════════════════════════════
    # ★ PRoPE: 在 block 循环之前设置，之后清除
    # ═══════════════════════════════════════════════════════
    if prope_data is not None and not use_unified_sequence_parallel:
        # USP 时暂不支持 PRoPE（需要额外处理 chunk offset）
        set_prope_data_on_blocks(dit, prope_data)
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                # ★ gradient_checkpoint_forward 的调用完全不变！
                #   PRoPE 数据通过 block.self_attn._prope_data 属性传递，
                #   不需要修改这里的参数
    ...
    # ★ PRoPE: 清除临时属性
    if prope_data is not None and not use_unified_sequence_parallel:
        clear_prope_data_on_blocks(dit)
