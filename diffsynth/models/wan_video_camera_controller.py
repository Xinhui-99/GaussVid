...
class DepthAdapter(nn.Module):
    """
    轻量版 depth adapter
    输入:  [B, C, F, H, W]，例如 [1, 32, 2, 560, 1024]
    输出:  [B, out_dim, F, H', W']，例如 [1, 5120, 2, 35, 64]
    """
    def __init__(
        self,
        in_dim=32,
        ...
        in_channels = in_dim * (downscale_factor ** 2)
        ...
        self.residual_blocks = nn.Sequential(
            *[LightweightResidualBlock(out_dim) for _ in range(num_residual_blocks)]
        )

        self.zero_conv = nn.Conv2d(out_dim, out_dim, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.pixel_unshuffle(x)   # spatial /8
        x = self.conv(x)              # 再 /2 -> 总共 /16
        x = self.residual_blocks(x)
        x = self.zero_conv(x)

        _, c_out, h_out, w_out = x.shape
        x = x.view(b, f, c_out, h_out, w_out).permute(0, 2, 1, 3, 4).contiguous()
        return x
...
class SimpleAdapter(nn.Module):
    def __init__(
        ...
        assert downscale_factor in [2, 4, 8], "downscale_factor 只建议使用 2/4/8"
        ...
        if isinstance(kernel_size, int):
            ...
            padding = tuple(0 for k in kernel_size)  # ← 改这里
    ...
    def forward(self, x):
        ...
        # [bs, c, f, h, w] -> [bs*f, c, h, w]
        x = x.permute(0, 2, 1, 3, 4).contiguous().view(bs * f, c, h, w)

        # pixel unshuffle
        x = self.pixel_unshuffle(x)

        # lightweight conv stem
        x = self.conv(x)
